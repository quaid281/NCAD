...
class CausalTCNBlock(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        x = self.conv1(x)
        x = F.gelu(x)
        x = self._layer_norm_time_first(x, self.norm1)
        x = self.dropout(x)

        x = F.pad(x, (self.padding, 0))
        x = self.conv2(x)
        x = F.gelu(x)
        x = self._layer_norm_time_first(x, self.norm2)
        x = self.dropout(x)
        return F.gelu(x + residual)

    def _layer_norm_time_first(self, x: torch.Tensor, norm: nn.LayerNorm) -> torch.Tensor:
        x = x.transpose(1, 2)
        x = norm(x)
