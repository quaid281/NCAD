...
The paper focuses on the primary univariate telemetry value from each channel.
This loader extracts that first column, normalizes with training statistics, and
...
class DataLoader:
    ...
    def load_channel(self, channel_id: str, normalize: bool = True) -> ChannelData:
        ...
        train_raw = self._extract_primary_signal(np.load(train_path))
        test_raw = self._extract_primary_signal(np.load(test_path))
    ...
    @staticmethod
    def _extract_primary_signal(values: np.ndarray) -> np.ndarray:
        if values.ndim == 1:
