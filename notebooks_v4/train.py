"""NCAD-CS v4: Counterfactual Successor Memory prototype."""
...
V3_DIR = PROJECT_ROOT / "notebooks_v3"
...
from v4_models.multi_scale_tcn_encoder import MultiScaleTCNEncoder
from v4_models.successor_memory import CounterfactualSuccessorMemory, SuccessorMemoryConfig
from v4_utils.event_fusion import (
...
def run_channel(channel_data: ChannelData, run_dir: Path, config: CSMConfig, device: torch.device) -> dict:
    ...
    print(f"[{channel_data.channel_id}] training {config.encoder_architecture} contrastive TCN encoder")
    model, training_history = train_encoder(train_windows, config, train_features.shape[1], device)

    print(f"[{channel_data.channel_id}] building Counterfactual Successor Memory")
    ...
    print(f"[{channel_data.channel_id}] scoring test windows with counterfactual successors")
...
def plot_channel_diagnostics(
    ...
    axes[3].plot(context_ood_map.astype(float), color="#7c3aed", drawstyle="steps-post", label="context OOD")
    axes[3].set_ylim(-0.05, 1.15)
    axes[3].set_title("Context reliability")
    axes[3].legend(loc="upper right")
    axes[3].set_xlabel("time step")
    fig.tight_layout()
    fig.savefig(channel_dir / f"{channel_data.channel_id}_successor_memory_diagnostics.png", dpi=160)
    plt.close(fig)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run NCAD-CS v4 Counterfactual Successor Memory.")
    ...
    parser.add_argument("--data-dir", type=str, default=None, help="Path to the data directory. Defaults to ../data from v3 utilities.")
    ...
    parser.add_argument(
        ...
        default="hybrid_tcn",
        help="Encoder architecture. The accepted paper configuration uses hybrid_tcn; multi_scale_tcn is experimental.",
    )
