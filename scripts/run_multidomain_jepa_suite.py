"""Multi-Domain Benchmark Suite: Evaluating TS-JEPA across Exathlon, room-occupancy, OPPORTUNITY, and SMAP."""
...
def main():
    ...
    # Dataset and Channel definitions
    benchmark_plan = {
    ...
    modes = ["NCAD_TCN", "TS_JEPA_Euclidean"]

    print("=" * 90)
    print(f"MULTI-DOMAIN BENCHMARK SUITE (TS-JEPA vs Baseline NCAD)")
    print(f"Datasets: {list(benchmark_plan.keys())} | Device: {device} | Epochs: {args.epochs}")
    print("=" * 90)

    all_results = []

    for ds_name, channels in benchmark_plan.items():
        ds_dir = ROOT / "mTSBench_data" / ds_name
        print(f"\n############################################################")
        print(f"DATASET: {ds_name} ({len(channels)} channels)")
        print(f"############################################################")

        for chan in channels:
            print(f"\n--- [{ds_name}] Channel: {chan} ---")
            if chan == "default":
            ...
            if not train_p.exists() or not test_p.exists():
                print(f"  Warning: Missing files for {chan} at {train_p}, skipping...")
                continue

            for mode in modes:
                print(f"  Running {mode:20s} ... ", end="", flush=True)
                ...
                print(f"Done in {res['elapsed_sec']}s | PA-F1: {pa_f1:.4f} | Oracle: {oracle_f1:.4f}")
                all_results.append(res)

    results_df = pd.DataFrame(all_results)
    out_csv = ROOT / "reports" / "multidomain_jepa_comparison.csv"
    results_df.to_csv(out_csv, index=False)
    print(f"\nSaved raw multi-domain results to {out_csv}")

    # Dataset Summary Pivot
    print("\n" + "=" * 90)
    print("MULTI-DOMAIN CONSOLIDATED PA-F1 PERFORMANCE:")
    print("=" * 90)
    summary = results_df.groupby(["dataset", "mode"])["pa_f1"].mean().unstack()
    summary["Lift (%)"] = ((summary["TS_JEPA_Euclidean"] - summary["NCAD_TCN"]) / (summary["NCAD_TCN"] + 1e-6)) * 100
    print(summary.to_string())

    oracle_summary = results_df.groupby(["dataset", "mode"])["oracle_pa_f1"].mean().unstack()
    print("\nORACLE PA-F1 CEILING BY DATASET:")
