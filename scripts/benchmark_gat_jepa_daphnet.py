"""Benchmark runner comparing TS-JEPA (TCN), Relational GAT-JEPA, and GAT-JEPA (Mahalanobis) on Daphnet."""
...
def train_and_eval_channel(
    ...
    if mode == "TS_JEPA_TCN":
    ...
    elif mode in ("GAT_JEPA", "GAT_JEPA_Mahalanobis"):
        ...
        use_mahal = (mode == "GAT_JEPA_Mahalanobis")
        if use_mahal:
            # Fit empirical covariance on training windows
    ...
    # Residual scoring & Fusion with local deviation
...
def main():
    ...
    print("=" * 80)
    print(f"TS-JEPA (TCN) vs Relational GAT-JEPA vs GAT-JEPA (Mahalanobis) on {args.dataset}")
    print(f"Device: {device} | Channels: {channels} | Epochs: {args.epochs}")
    print("=" * 80)

    all_results = []
    modes = ["TS_JEPA_TCN", "GAT_JEPA", "GAT_JEPA_Mahalanobis"]
    ...
    out_csv = ROOT / "reports" / "daphnet_gat_jepa_comparison.csv"
