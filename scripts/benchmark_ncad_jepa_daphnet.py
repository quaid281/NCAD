"""Benchmark runner comparing Baseline NCAD-TCN, Pure TS-JEPA, and NCAD-JEPA Fusion on Daphnet."""
...
def train_and_eval_channel(
    ...
    if mode == "NCAD_TCN":
        ...
        for epoch in range(1, epochs + 1):
            ...
            with torch.no_grad():
                for b in range(0, len(val_data), batch_size):
                    ...
                    val_loss += float(vl.item()) * len(clean)
        ...
        # Calibrate with normal training scores
        ...
        # Score via Predictive Discrepancy
        def compute_discrepancy(arr):
            ...
            with torch.no_grad():
                for i in range(0, len(arr), 2048):
                    ...
                    disc = model.compute_predictive_discrepancy(ctx, tgt).cpu().numpy()
                    res.append(disc)
        ...
        # Unified Architecture: TCN + JEPA + Injections + CSM Memory
        model = NCADJEPAModel(
            input_dim=input_dim,
        ...
        for epoch in range(1, epochs + 1):
            ...
            for b in range(0, len(perm), batch_size):
                ...
                clean_ctx = torch.from_numpy(clean[:, :context_size]).float().to(device)
                clean_tgt = torch.from_numpy(clean[:, context_size:]).float().to(device)
                inj_ctx = torch.from_numpy(mod).float().to(device)
                inj_lbl = torch.from_numpy(lbl).float().to(device)

                total_loss, _ = model.compute_joint_loss(
                    clean_context=clean_ctx,
                    clean_target=clean_tgt,
                    injected_context=inj_ctx,
                    injected_label=inj_lbl,
                )
                optimizer.zero_grad(set_to_none=True)
                total_loss.backward()
                optimizer.step()
        ...
        # Build Counterfactual Successor Memory in joint latent space
        def encode_c(arr):
            model.eval()
            res = []
            with torch.no_grad():
                for i in range(0, len(arr), 2048):
                    t = torch.from_numpy(arr[i : i + 2048]).float().to(device)
                    res.append(model.encode_context(t).cpu().numpy())
            return np.concatenate(res, axis=0)
        ...
        def encode_t(arr):
            ...
            with torch.no_grad():
                for i in range(0, len(arr), 2048):
                    t = torch.from_numpy(arr[i : i + 2048]).float().to(device)
                    res.append(model.encode_target(t).cpu().numpy())
            return np.concatenate(res, axis=0)
        ...
        train_ctx_emb = encode_c(train_windows[:, :context_size])
        train_suc_emb = encode_t(train_windows[:, context_size:])
        memory = CounterfactualSuccessorMemory(SuccessorMemoryConfig(n_neighbors=8, max_memory_windows=4000, seed=42))
        memory.fit(train_ctx_emb, train_suc_emb)
        ...
        # Test inference
        test_ctx_emb = encode_c(test_windows[:, :context_size])
        test_suc_emb = encode_t(test_windows[:, context_size:])
        query = memory.query(test_ctx_emb, test_suc_emb)
        test_disc = compute_discrepancy(test_windows)
        test_local = local_deviation_scores(test_windows, context_size)
        ...
        # Tri-evidence fusion: 50% JEPA physical discrepancy + 50% CSM counterfactual
        fused_core = 0.5 * disc_z + 0.5 * succ_z
        win_scores = fuse_evidence_scores(successor_z=fused_core, local_z=loc_z, context_ratio=ctx_ratio)

        # Training normal score calibration
        train_disc_z = positive_robust_z(train_disc[memory.sample_indices], disc_stats, clip=20.0)
        train_succ_z = positive_robust_z(memory.calibration_successor_scores, succ_stats, clip=20.0)
        train_loc_z = positive_robust_z(train_local[memory.sample_indices], local_stats, clip=20.0)
        train_ctx_ratio = np.minimum(memory.calibration_context_distances / max(float(memory.context_threshold), 1e-6), 3.0)
        train_fused_core = 0.5 * train_disc_z + 0.5 * train_succ_z
        train_win_scores = fuse_evidence_scores(successor_z=train_fused_core, local_z=train_loc_z, context_ratio=train_ctx_ratio)
    else:
...
def main():
    ...
    # Select standard 6 benchmark channels
    bench_channels = ["S01R01E1", "S02R01E0", "S02R02E0", "S03R01E0", "S03R01E1", "S03R02E0"]
    channels = [c for c in bench_channels if c in channels]

    print("=" * 80)
    print(f"NCAD-TCN vs TS-JEPA vs NCAD-JEPA Fusion Benchmark on {args.dataset}")
    print(f"Device: {device} | Channels: {channels} | Epochs: {args.epochs}")
    print("=" * 80)

    all_results = []
    modes = ["NCAD_TCN", "TS_JEPA", "NCAD_JEPA"]
    ...
    for chan in channels:
        ...
        for mode in modes:
            print(f"  Running {mode:12s} ... ", end="", flush=True)
    ...
    out_csv = ROOT / "reports" / "daphnet_ncad_jepa_comparison.csv"
    ...
    for mode in modes:
        ...
        print(f"  {mode:12s} -> Mean PA-F1: {avg_pa:.4f} | Mean Point-F1: {avg_pt:.4f} | Mean Oracle PA-F1: {avg_oracle:.4f}")
    print("=" * 80)
