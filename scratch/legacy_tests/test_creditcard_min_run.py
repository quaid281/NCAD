...
def evaluate_threshold_strategy(min_run, name):
    ...
    val_smoothed = moving_average(val_point_scores, smoothing_window)
    
    test_point_scores, test_mask = aggregate_window_scores(successor_z, len(test_df), context_size, suspect_size, step)
    test_smoothed = moving_average(test_point_scores, smoothing_window)
    
    # Search threshold on validation using wider search
    val_valid_scores = val_smoothed[val_mask]
    best_f1 = 0
    best_th = 0
    # Wide search
    candidates = np.linspace(np.percentile(val_valid_scores, 1.0), np.percentile(val_valid_scores, 99.99), 300)
    for th in candidates:
        preds = event_level_filter(val_smoothed, th, val_mask, min_run=min_run, extreme_factor=1.75) * val_mask.astype(np.float32)
    ...
    # Apply to test set
    test_preds = event_level_filter(test_smoothed, best_th, test_mask, min_run=min_run, extreme_factor=1.75) * test_mask.astype(np.float32)
...
evaluate_threshold_strategy(min_run=2, name="Default Sequence Filter")
