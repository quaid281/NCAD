...
class UnidirectionalGRUBlock(nn.Module):
...
class UnidirectionalSSMEncoder(nn.Module):
    def __init__(
        self,
        input_dim: int,
        latent_dim: int = 16,
        ...
        self.blocks = nn.ModuleList(
            [UnidirectionalGRUBlock(hidden_dim, dropout=dropout) for _ in range(layers)]
        )
...
def run_unidirectional(chan):
    print(f"\n--- Running Unidirectional Encoder on Channel {chan} ---")
    ...
    model = UnidirectionalSSMEncoder(
    ...
    print(f"Oracle Standard F1: {best_std_f1:.4f}")
    print(f"Oracle PA F1:       {best_pa_f1:.4f}")

if __name__ == "__main__":
    run_unidirectional('1')
