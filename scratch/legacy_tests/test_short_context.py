...
# 3. Sliding Windows with SHORT Context
context_size = 24  # 1 day instead of 12 days
suspect_size = 2   # 2 hours instead of 16 hours
...
# Build encoder with fewer layers since context is smaller (dilation matches shorter context)
model = HybridTCNEncoder(input_dim=input_dim, latent_dim=16, filters=64, tcn_layers=3, kernel_size=3, dropout=0.20)
...
def local_deviation_scores_multivariate(windows, context_size, tail_size=12):
    scores = []
    # Check all features in a fast, vectorized way
    raw_values = np.asarray(windows, dtype=np.float64)
    ...
    point_z = np.abs((suspects - medians[:, None, :]) / scale[:, None, :])
    max_point_z = np.max(point_z, axis=(1, 2))
    
    mean_shift = np.abs(np.mean(suspects, axis=1) - medians) / scale
    max_mean_shift = np.max(mean_shift, axis=1)
    return np.maximum(max_point_z, max_mean_shift).astype(np.float32)

# Compute Anomaly Scores
def compute_anomaly_scores(windows, dataset_name):
    train_local_scores = local_deviation_scores_multivariate(train_windows, context_size, tail_size=12)
    ...
    local_raw_scores = local_deviation_scores_multivariate(windows, context_size, tail_size=12)
...
def process_point_scores(window_scores, raw_len):
    ...
    smoothed = moving_average(point_scores, 4) # smaller smoothing window for short context
...
# 8. Threshold search on Val and eval on Test
...
best_f1 = 0.0
best_threshold = 0.0
candidates = np.linspace(np.percentile(val_valid_scores, 50.0), np.percentile(val_valid_scores, 99.9), 150)
...
# Check optimal test F1 directly for upper bound
best_test_f1 = 0.0
best_test_th = 0.0
for th in np.linspace(np.percentile(test_scores[test_mask], 50.0), np.percentile(test_scores[test_mask], 99.9), 150):
    preds = event_level_filter(test_scores, th, test_mask, min_run=2, extreme_factor=1.75)
    preds = preds * test_mask.astype(np.float32)
    metrics = compute_metrics(test_labels, preds, valid_mask=test_mask)
