...
# Compare Unnormalized Z-score Fusion vs Normalized Z-score Fusion
for norm in [False, True]:
    print(f"\n--- NORMALIZED Z-FUSION = {norm} ---")
    if norm:
        # Scale-normalize successor_z and local_z by their 95th percentile on calibration/background
        succ_scale = max(float(np.percentile(successor_z, 95.0)), 1e-4)
        local_scale = max(float(np.percentile(local_z, 95.0)), 1e-4)
        norm_successor_z = successor_z / succ_scale
        norm_local_z = local_z / local_scale
        window_scores = fuse_evidence_scores(successor_z=norm_successor_z, local_z=norm_local_z, context_ratio=context_ratio)
    else:
