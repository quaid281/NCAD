import os
# Limit CPU threads to be extremely lightweight and quiet
...
print(f"Executing test on device: {device}", flush=True)
...
def local_deviation_scores_custom(windows, context_size, tail_size=64, mad_floor=1e-4):
...
def run_experiment(std_floor=0.0, mad_floor=1e-4):
    ...
    if std_floor > 0.0:
        ...
        # Standard StandardScaler behavior (divide by std, handle constant with 1.0)
        scale = train_std.copy()
        scale[scale == 0.0] = 1.0
        
    train_scaled = (train_df[numeric_cols].to_numpy() - train_mean) / scale
    test_scaled = (test_df[numeric_cols].to_numpy() - train_mean) / scale
    
    # Subsample train windows with stride=6 to speed up CPU training
    train_windows = DataLoader.create_windows(train_scaled, window_size, step=6)
    ...
    # Train (1 epoch is enough for subsampled context representation learning)
    epochs = 1
    ...
    print(f"Std floor: {std_floor} | MAD floor: {mad_floor} | Unsub PA F1: {m_unsub_pa.get('f1', 0.0):.4f} | Oracle PA F1: {best_pa_f1:.4f}", flush=True)
    print(f"  Test scores - Min: {test_scores[valid_mask].min():.4f}, Max: {test_scores[valid_mask].max():.4f}, Mean: {test_scores[valid_mask].mean():.4f}", flush=True)

print("=== Running Baseline Experiment (StandardScaler, local deviation floor 1e-4) ===", flush=True)
run_experiment(std_floor=0.0, mad_floor=1e-4)

print("\n=== Running Experiment 1 (std_floor=0.05, mad_floor=0.1) ===", flush=True)
run_experiment(std_floor=0.05, mad_floor=0.1)

print("\n=== Running Experiment 2 (std_floor=0.1, mad_floor=0.2) ===", flush=True)
run_experiment(std_floor=0.1, mad_floor=0.2)
