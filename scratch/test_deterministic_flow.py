"""Test deterministic trajectory and fixed-time flow discrepancy for EVT."""
...
def test_deterministic_flow_discrepancy():
    ...
    # Method 1: Trajectory ODE Discrepancy (Deterministic prior z_0 = 0)
    def compute_trajectory_disc(arr):
        model.eval()
        res = []
        with torch.no_grad():
            for i in range(0, len(arr), 1024):
                ...
                z_gen = model.sample_target_patches(ctx, n_steps=2, solver="midpoint", z_init=z_init)
                diff = torch.linalg.norm(z_tgt_obs - z_gen, dim=-1).mean(dim=-1)
                res.append(diff.cpu().numpy())
        return np.concatenate(res, axis=0)

    # Method 2: Midpoint Velocity Field Discrepancy (Deterministic t=0.5, z_0 = 0)
    def compute_midpoint_disc(arr):
        model.eval()
        res = []
        with torch.no_grad():
            for i in range(0, len(arr), 1024):
                ...
                z_tgt_obs = model.target_encoder(tgt)
                # At t=0.5, z_t = 0.5 * z_tgt_obs
                t_mid = torch.full((len(ctx),), 0.5, device=device)
                z_mid = 0.5 * z_tgt_obs
                v_pred = model.flow_predictor(z_mid, t_mid, h_ctx)
                # Target velocity u_t = z_tgt_obs - 0 = z_tgt_obs
                diff = torch.linalg.norm(v_pred - z_tgt_obs, dim=-1).mean(dim=-1)
                res.append(diff.cpu().numpy())
        return np.concatenate(res, axis=0)

    for name, disc_fn in [("Trajectory (ODE)", compute_trajectory_disc), ("Midpoint Velocity Field", compute_midpoint_disc)]:
        train_windows_dense = DataLoader.create_windows(train_scaled, window_size, step=1)
        train_disc_dense = disc_fn(train_windows_dense)
        ...
        print(f"\n[{name}]")
        print(f"  Train: median={disc_stats.median:.4f}, iqr={disc_stats.iqr:.4f}")
        print(f"  EVT Thresh: {evt_th:.4f} -> PA-F1: {m_pa.get('f1', 0.0):.4f} (P: {m_pa.get('precision', 0.0):.4f}, R: {m_pa.get('recall', 0.0):.4f})")
        print(f"  Oracle Ceiling: {best_pa_f1:.4f}")

if __name__ == "__main__":
    test_deterministic_flow_discrepancy()
