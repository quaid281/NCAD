...
# 5. Custom Successor Memory that handles multivariate without dilution
class MultivariateCounterfactualSuccessorMemory(CounterfactualSuccessorMemory):
    def _score_against_neighbors(self, observed_successors, distances, neighbor_indices):
        ...
        # Calculate time-MSE for each feature: shape (batch, n_neighbors, n_features)
        feature_mse = np.mean((neighbor_successors - observed_successors[:, None, :, :]) ** 2, axis=2)
        
        # Take max feature MSE to prevent multivariate dilution (any feature anomaly stands out)
        max_feature_mse = np.max(feature_mse, axis=2)
        residuals = np.sqrt(max_feature_mse).astype(np.float32)
        
        # Expected successors for median stats
        ...
        # Multivariate distance to expected successors
        expected_diff = (expected_successors - observed_successors) ** 2
        successor_median_scores = np.sqrt(np.max(np.mean(expected_diff, axis=1), axis=1)).astype(np.float32)
...
memory = MultivariateCounterfactualSuccessorMemory(
...
# 6. Vectorized, All-Feature Multivariate Local Deviation Score
