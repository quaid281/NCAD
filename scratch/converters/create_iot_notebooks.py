...
def generate_notebook_dict(
    dataset_name, dataset_dir_name, file_prefix, 
    context_size, suspect_size, min_inj, max_inj, 
    train_step, clip=20.0, drop_cols=None
):
    cells = []
    ...
    cells.append({
        ...
        "source": [
            ...
            f"3. **Performance-Optimized Windowing**: Sequence windows of size `context_size + suspect_size` ({context_size + suspect_size} steps) are created using stride tricks.\n",
    ...
    exclude_list = ['timestamp', 'is_anomaly'] + (drop_cols or [])
    cells.append({
        "cell_type": "code",
        "execution_count": None,
        "id": "core_loop",
        ...
        "source": [
            ...
            "    fname = Path(f).name\n",
            f"    if fname == '{file_prefix}_train.csv':\n",
            "        chan = ''\n",
            "    else:\n",
            f"        chan = fname.replace('{file_prefix}_', '').replace('_train.csv', '')\n",
            ...
            "    # Load CSVs\n",
            "    if chan == '':\n",
            f"        train_path = dataset_dir / '{file_prefix}_train.csv'\n",
            f"        test_path = dataset_dir / '{file_prefix}_test.csv'\n",
            "    else:\n",
            ...
            "    \n",
            f"    numeric_cols = [c for c in train_df.columns if c not in {exclude_list}]\n",
            ...
            f"    injector = ContextualAnomalyInjector(AnomalyInjectionConfig(injection_ratio=injection_ratio, min_anomaly_len={min_inj}, max_anomaly_len={max_inj}), seed=42)\n",
            "    \n",
            ...
            "    \n",
            f"    successor_z = positive_robust_z(query.successor_scores, successor_stats, clip={clip})\n",
            f"    local_z = positive_robust_z(test_local_scores, local_stats, clip={clip})\n",
            "    \n",
...
# Dynamic, optimized window sizes and anomaly injection ranges based on segment analysis
iot_datasets = [
    # (Name, Dir, Prefix, context, suspect, min_inj, max_inj, train_step, clip, drop_cols)
    ("Daphnet", "Daphnet", "Daphnet", 256, 128, 32, 128, 10, 20.0, None),
    ("GECCO", "GECCO", "GECCO_water_quality", 64, 8, 2, 8, 6, 20.0, ["Tp"]),
    ("Genesis", "Genesis", "Genesis", 64, 8, 2, 8, 1, 20.0, None),
    ("GHL", "GHL", "GHL", 512, 128, 32, 128, 30, 20.0, None),
    ("OPPORTUNITY", "OPPORTUNITY", "OPPORTUNITY", 256, 128, 32, 128, 10, 20.0, None),
    ("room-occupancy", "room-occupancy", "room-occupancy", 128, 32, 8, 32, 1, 500.0, None),
]

for name, dir_name, prefix, context, suspect, min_inj, max_inj, train_step, clip, drop_cols in iot_datasets:
    # Ensure folder under notebooks_v4 exists
    folder = Path("notebooks_v4") / dir_name
    folder.mkdir(parents=True, exist_ok=True)
    
    nb_dict = generate_notebook_dict(
        name, dir_name, prefix, 
        context_size=context, suspect_size=suspect, 
        min_inj=min_inj, max_inj=max_inj, 
        train_step=train_step, clip=clip, drop_cols=drop_cols
    )
    nb_file = folder / f"{name}_SSM_Anomaly_Detection.ipynb"
