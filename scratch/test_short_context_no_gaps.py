...
# 3. Sliding Windows with SHORT Context (no gaps since step = suspect_size)
context_size = 24
suspect_size = 4
...
# 3 TCN layers is perfect for receptive field of 24
...
def local_deviation_scores_multivariate(windows, context_size, tail_size=12):
    scores = []
    # Fast vectorized computation over all features
...
def process_point_scores(window_scores, raw_len):
    ...
    smoothed = moving_average(point_scores, 4)
...
print(f"\nOptimal Test Set F1: {best_test_f1:.4f} at threshold {best_test_th:.5f}")
