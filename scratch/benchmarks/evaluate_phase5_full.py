...
for mode in ["contrastive", "fei_sigreg"]:
    print(f"\n=================== PRE-TRAINING MODE: {mode.upper()} ===================")
    ...
    if mode == "contrastive":
        ...
        for epoch in range(1, 16):
            ...
            for batch_start in range(0, len(epoch_indices), 32):
                batch_indices = epoch_indices[batch_start : batch_start + 32]
                clean_batch = training_data[batch_indices]
                clean_tensor = torch.from_numpy(clean_batch).float().to(device)
                masked_tensor = masker.mask_batch(clean_tensor)
                optimizer.zero_grad(set_to_none=True)
                loss = sigreg_loss(model(masked_tensor), model(clean_tensor))
                loss.backward()
                optimizer.step()
    ...
    window_scores = fuse_evidence_scores(successor_z=successor_z, local_z=local_z, context_ratio=context_ratio, normalize_components=True)
    ...
    print(f"Successor Noise Mean: {np.mean(query.successor_scores):.5f}, Std: {np.std(query.successor_scores):.5f}")
