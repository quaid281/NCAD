...
def main():
    notebooks = [
        project_root / "notebooks_v4" / "room-occupancy" / "room-occupancy_SSM_Anomaly_Detection.ipynb",
        project_root / "notebooks_v4" / "GHL" / "GHL_SSM_Anomaly_Detection.ipynb",
    ...
    print(f"Running {len(notebooks)} optimized notebooks with max_workers=3 concurrently...", flush=True)
    
    start_total = time.time()
    with ProcessPoolExecutor(max_workers=3) as executor:
        futures = {executor.submit(execute_notebook, nb): nb for nb in notebooks if nb.exists()}
    ...
    print(f"All optimized notebooks finished in {time.time() - start_total:.2f}s.", flush=True)
