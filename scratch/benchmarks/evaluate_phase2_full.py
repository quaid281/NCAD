...
def run_experiment(use_delay=True, successor_space="latent"):
    print(f"\n=================== RUNNING: DelayEmbedding={use_delay}, SuccessorSpace={successor_space.upper()} ===================")
    feature_cfg = FeatureConfig(
        max_features=64,
        use_delay_embedding=use_delay,
    ...
    return {
        "score_std": float(np.std(query.successor_scores)),
        "score_mean": float(np.mean(query.successor_scores)),
...
results = {}
results["Baseline (Raw, No Delay)"] = run_experiment(use_delay=False, successor_space="raw")
results["Phase 1 (Latent, No Delay)"] = run_experiment(use_delay=False, successor_space="latent")
results["Phase 1 + Phase 2 (Latent + Delay)"] = run_experiment(use_delay=True, successor_space="latent")

print("\n=================== FINAL COMPARATIVE RESULTS ===================")
for name, res in results.items():
    print(f"\n--- CONFIG: {name} ---")
    print(f"Successor Noise Mean: {res['score_mean']:.5f}, Std: {res['score_std']:.5f}")
    print(f"Elbow Threshold: {res['threshold']:.5f}")
    print("Standard Metrics (No PA):")
    print(f"  Precision: {res['metrics_std'].get('precision', 0):.4f}")
    print(f"  Recall:    {res['metrics_std'].get('recall', 0):.4f}")
    print(f"  F1-Score:  {res['metrics_std'].get('f1', 0):.4f}")
    print(f"  Confusion: TP={res['metrics_std'].get('tp')}, FP={res['metrics_std'].get('fp')}, TN={res['metrics_std'].get('tn')}, FN={res['metrics_std'].get('fn')}")
    print("Point-Adjusted Metrics (PA):")
    print(f"  Precision: {res['metrics_pa'].get('precision', 0):.4f}")
    print(f"  Recall:    {res['metrics_pa'].get('recall', 0):.4f}")
    print(f"  F1-Score:  {res['metrics_pa'].get('f1', 0):.4f}")
