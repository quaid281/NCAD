...
print("\nTraining HybridTCN Encoder on CalIt2 (15 epochs)...")
...
for mode in ["raw", "latent"]:
    print(f"\n=================== CALIT2 EVALUATION: MODE = {mode.upper()} ===================")
    if mode == "latent":
...
for mode, res in results_summary.items():
    print(f"\n--- MODE: {mode.upper()} ---")
    print(f"Successor Score Noise Mean: {res['score_mean']:.5f}, Std: {res['score_std']:.5f}")
    print(f"Adaptive Elbow Threshold: {res['threshold']:.5f}")
