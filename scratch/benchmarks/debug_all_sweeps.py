...
def run_debug(dataset_name, dir_name, file_prefix, chan):
    print(f"\n=========================================")
    print(f"DEBUGGING: {dataset_name} (Channel: {chan})")
    print(f"=========================================")
    
    dataset_dir = project_root / 'mTSBench_data' / dir_name
    if chan == '':
        train_path = dataset_dir / f"{file_prefix}_train.csv"
        test_path = dataset_dir / f"{file_prefix}_test.csv"
    else:
        train_path = dataset_dir / f"{file_prefix}_{chan}_train.csv"
        test_path = dataset_dir / f"{file_prefix}_{chan}_test.csv"
    ...
    training_data = train_windows[indices[n_val:]]
    ...
    # 1. Evaluate with clip=20.0 (Standard)
    run_with_clip(test_df, test_windows, model, memory, train_windows, test_labels, context_size, suspect_size, step, clip=20.0, label="Standard (clip=20.0)")
    # 2. Evaluate with clip=500.0 (Expanded)
    run_with_clip(test_df, test_windows, model, memory, train_windows, test_labels, context_size, suspect_size, step, clip=500.0, label="Expanded (clip=500.0)")

def run_with_clip(test_df, test_windows, model, memory, train_windows, test_labels, context_size, suspect_size, step, clip, label):
    ...
    print(f"Unsupervised Standard F1: {m_unsub_std.get('f1', 0.0):.4f} | PA F1: {m_unsub_pa.get('f1', 0.0):.4f}")
    
    valid_scores = test_scores[valid_mask]
    print(f"Scores Info - Min: {np.min(valid_scores):.4f}, Max: {np.max(valid_scores):.4f}, 50%: {np.percentile(valid_scores, 50.0):.4f}, 99%: {np.percentile(valid_scores, 99.0):.4f}")
    
    # Compare sweeps
    # Sweep A: Old capped linear search (50% to max)
    candidates_a = np.linspace(np.percentile(valid_scores, 50.0), np.max(valid_scores), 100)
    best_pa_f1_a = 0.0
    for th in candidates_a:
        ...
        best_pa_f1_a = max(best_pa_f1_a, m_pa.get('f1', 0.0))
        
    # Sweep B: New full percentile search (0% to 100% percentile, unique, including unsub threshold)
    candidates_b = np.percentile(valid_scores, np.linspace(0.0, 100.0, 200))
    candidates_b = np.unique(np.concatenate([candidates_b, [unsupervised_threshold]]))
    best_pa_f1_b = 0.0
    for th in candidates_b:
        preds = event_level_filter(test_scores, th, valid_mask, min_run=2, extreme_factor=1.75) * valid_mask.astype(np.float32)
        m_pa = compute_metrics(test_labels, preds, valid_mask=valid_mask, use_pa=True)
        best_pa_f1_b = max(best_pa_f1_b, m_pa.get('f1', 0.0))
        
    print(f"Sweep A (Linear 50%-Max) - Unique candidates: {len(np.unique(candidates_a))}, Best PA F1: {best_pa_f1_a:.4f}")
    print(f"Sweep B (Percentile 0%-100%) - Unique candidates: {len(candidates_b)}, Best PA F1: {best_pa_f1_b:.4f}")
...
if __name__ == "__main__":
    # Test on room-occupancy (Channel 1)
    run_debug("room-occupancy", "room-occupancy", "room-occupancy", "1")
    # Test on Genesis (Channel default)
    run_debug("Genesis", "Genesis", "Genesis", "")
