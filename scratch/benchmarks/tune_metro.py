...
# Load Data
dataset_dir = project_root / 'mTSBench_data' / 'metro'
train_path = dataset_dir / 'metro_traffic-volume_train.csv'
test_path = dataset_dir / 'metro_traffic-volume_test.csv'
...
print("Fitting Successor Memory...")
...
# Sweep over clip and other parameters
for clip in [20.0, 100.0, 500.0]:
    ...
    for min_run in [1, 2]:
        for extreme_factor in [1.0, 1.75]:
            best_pa_f1 = 0.0
            best_std_f1 = 0.0
            for th in candidates:
                preds = event_level_filter(test_scores, th, valid_mask, min_run=min_run, extreme_factor=extreme_factor) * valid_mask.astype(np.float32)
            ...
            print(f"Clip: {clip} | MinRun: {min_run} | ExtFactor: {extreme_factor} | Best Oracle PA F1: {best_pa_f1:.4f} | Best Oracle Std F1: {best_std_f1:.4f}")
