...
# Create windows directly from scaled raw features (only 2 features!)
context_size = 284
suspect_size = 16
...
local_raw_scores = local_deviation_scores_multivariate(test_windows, context_size, tail_size=64)
...
def evaluate(window_scores, name):
    point_scores, valid_mask = aggregate_window_scores(
        window_scores, n_points=len(test_df), context_size=context_size, suspect_size=suspect_size, step=1
    )
...
print("=== CalIt2 Raw-only Features Evaluation ===")
evaluate(fused_scores, "Original Fused")
evaluate(successor_z, "Successor Z Only")
evaluate(local_z, "Multivariate Local Z Only")
