"""Unit tests for MultiScaleLatentWorldJEPA (Multi-Scale Dilated Latent World Model)."""
...
def test_multiscale_latent_core_step():
    B, D, H = 4, 16, 32
    core = MultiScaleLatentCore(latent_dim=D, hidden_dim=H)

    z_0 = torch.randn(B, D)
    h_fast = torch.zeros(B, H)
    h_med = torch.zeros(B, H)
    h_slow = torch.zeros(B, H)

    z_hat_next, h_f, h_m, h_s = core.step(z_0, h_fast, h_med, h_slow, step_idx=0)
    assert z_hat_next.shape == (B, D)
    assert h_f.shape == (B, H)
    assert h_m.shape == (B, H)
    assert h_s.shape == (B, H)


def test_multiscale_latent_world_jepa_forward_loss_and_discrepancy():
    B, C_len, H_len, C_dim = 4, 32, 16, 3
    latent_dim = 16

    model = MultiScaleLatentWorldJEPA(input_dim=C_dim, latent_dim=latent_dim, hidden_dim=32)
