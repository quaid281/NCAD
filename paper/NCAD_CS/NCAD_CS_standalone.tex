...
To resolve raw-space numerical noise bottlenecks and the reconstruction/collapse traps of deep baselines, we introduce Point-JEPA and its continuous flow generalization Flow-JEPA (Time-Series Flow-Matching Joint-Embedding Predictive Architecture). Rather than attempting to integrate stiff trajectories in raw sensor space or reconstructing raw waveforms, our framework embeds the system into an abstract representation space and directly learns the continuous velocity vector field of the nominal physical manifold via Optimal Transport Conditional Flow Matching (OT-CFM) \cite{lipman2022flow}.
...
\end{equation}
Rather than assuming an unregularized bound on network curvature $\text{Tr}(\nabla_z^2 v_\psi)$, we evaluate the empirical magnitude of the second-order bias directly by measuring the discrepancy between $\hat{v}_{\text{mid}}$ and $K=10$ Monte Carlo sampled draws $\frac{1}{K}\sum_{k=1}^K v_\psi(z_{0.5}^{(k)}, 0.5, z_{\text{ctx}})$. As demonstrated in our empirical validation (Section~\ref{sec:ablations} and Fig.~\ref{fig:flow_jepa_validation}), the deterministic plug-in score tracks the Monte Carlo expectation with $R^2 = 0.9993$ and a mean relative score deviation of $<0.8\%$ on the isolated Daphnet benchmark sequence. This confirms that the second-order correction term is practically negligible in practice, while eliminating Monte Carlo sampling variance ($\text{Var} = 0$) and delivering a $10\times$ inference speedup ($3.47$s single-pass vs. $34.2$s for $K=10$ sampling).
...
\begin{enumerate}[leftmargin=*]
    ...
    \end{equation}
    \item \textbf{Flow-JEPA (Proposed Primary Architecture):} Employs dilated causal TCN encoders with continuous Optimal Transport Conditional Flow Matching ($\mathcal{L}_{\text{CFM}}$) predicting velocity field $v_\psi(z_t, t, z_{\text{ctx}})$ along straight probability paths, regularized by Spectral Covariance:
    \begin{equation}
        \mathcal{L}_{\text{Flow-JEPA}} = \mathcal{L}_{\text{CFM}} + \lambda_{\text{var}}\mathcal{L}_{\text{var}} + \lambda_{\text{cov}}\mathcal{L}_{\text{cov}}.
    \end{equation}
    ...
    \end{equation}
    \item \textbf{NCAD-Flow:} Augments the continuous flow velocity objective with Contextual Outlier Exposure (COE) \cite{carmona2021neural} by generating synthetic corrupted context windows $\tilde{x}_{\text{ctx}}$ and enforcing a discriminative margin loss:
    \begin{equation}
        \mathcal{L}_{\text{NCAD-Flow}} = \mathcal{L}_{\text{CFM}}(x_{\text{ctx}}, x_{\text{tgt}}) + \alpha \max\left(0,\, \gamma - \|v_\psi(\bar{z}_{0.5}, 0.5, \tilde{z}_{\text{ctx}}) - z_{\text{tgt}}\|_2^2\right) + \lambda_{\text{reg}}\mathcal{L}_{\text{reg}},
    \end{equation}
    which explicitly penalizes the velocity network from matching corrupted trajectories to nominal physical targets.
\end{enumerate}
...
\caption{Empirical Validation and Diagnostic Analysis of Flow-JEPA. 
...
(d) Latency-accuracy Pareto frontier comparing ODE solvers, Monte Carlo flow, Point-JEPA, and Flow-JEPA ($3.47$s / $0.9209$ F1).\label{fig:flow_jepa_validation}}
\end{figure*}

\subsection{Ablation 4: Flow Discrepancy Scoring vs. Numerical ODE Solvers}
Table~\ref{tab:ablations}(d) and Fig.~\ref{fig:flow_jepa_validation} evaluate anomaly scoring formulations on an isolated, high-SNR benchmark sequence (Daphnet \texttt{S03R02E0} Freezing-of-Gait telemetry). This sequence was specifically selected to isolate the mathematical properties and numerical precision of continuous ODE solvers vs. single-step velocity estimators without low-SNR measurement noise confounding the numerical comparison:
\begin{enumerate}[noitemsep, topsep=0pt, leftmargin=*]
    \item \textit{Deterministic Midpoint Velocity ($t=0.5$):} Directly evaluates $\|v_\psi(0.5 z_{\text{tgt}}, 0.5, z_{\text{ctx}}) - z_{\text{tgt}}\|_2$. As shown in Fig.~\ref{fig:flow_jepa_validation}(b) and (d), it provides a sharp proxy for trajectory divergence without Monte Carlo sampling variance, achieving high precision ($0.9861$), peak F1 ($0.9209$), and minimal inference latency ($3.47$s).
