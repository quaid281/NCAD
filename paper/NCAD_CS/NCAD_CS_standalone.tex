...
\end{equation}
Because the velocity network is regularized over a smooth latent manifold (where the Hessian trace $\text{Tr}(\nabla_z^2 v_\psi)$ remains bounded), the deterministic midpoint $\hat{v}_{\text{mid}}$ serves as an accurate, low-bias first-order plug-in estimator. Crucially, it eliminates Monte Carlo sampling variance ($\text{Var} = 0$) and enables single-forward-pass scoring ($3.47$s vs. $34.2$s for $K=10$ Monte Carlo sampling), with empirical validation in Section~\ref{sec:ablations} showing $<0.8\%$ score deviation on nominal manifolds.
...
\begin{enumerate}[leftmargin=*]
    ...
    \end{equation}
    \item \textbf{Patch-JEPA (Patch Transformer Baseline):} Segments context and suspect time series into non-overlapping temporal patches $P \in \mathbb{R}^{K \times D_{\text{in}}}$, maps tokens through multi-head self-attention Transformer blocks \cite{vaswani2017attention, dosovitskiy2020image}, and optimizes deterministic patch embedding prediction with VICReg regularization.
    \item \textbf{Flow-JEPA (Proposed Primary Architecture):} Employs dilated causal TCN encoders with continuous Optimal Transport Conditional Flow Matching ($\mathcal{L}_{\text{CFM}}$) predicting velocity field $v_\psi(z_t, t, z_{\text{ctx}})$ along straight probability paths, regularized by Spectral Covariance Entropy.
    \item \textbf{Patch-Flow:} Combines the patch-tokenized Transformer encoder with continuous Optimal Transport Flow Matching and Spectral Covariance Regularization.
    \item \textbf{NCAD-Flow:} Extends Neural Contextual Anomaly Detection \cite{carmona2021neural} by augmenting the continuous flow velocity objective with Contextual Outlier Exposure (COE), injecting synthetic anomalies as negative velocity trajectories during training.
\end{enumerate}
